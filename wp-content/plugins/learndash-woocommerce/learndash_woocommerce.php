...
class Learndash_WooCommerce {
	...
	public static function setup_constants() {
		if ( ! defined( 'LEARNDASH_WOOCOMMERCE_VERSION' ) ) {
			define( 'LEARNDASH_WOOCOMMERCE_VERSION', '1.9.0' );
		}
				...
				foreach ( $products as $product ) {
					...
					if ( $courses_id && is_array( $courses_id ) ) {
						foreach ( $courses_id as $cid ) {
							...
							update_user_meta( $order->get_user_id(), 'course_' . $cid . '_access_from', strtotime( $order->get_date_created() ) );
						}
					}
					...
					if ( $groups_id && is_array( $groups_id ) ) {
						foreach ( $groups_id as $group_id ) {
							...
							// Reset enrollment date to order date
							update_user_meta( $order->get_user_id(), 'learndash_group_enrolled_' . $group_id, strtotime( $order->get_date_created() ) );
						}
					}
				}
			}
	...
	public static function check_subscription_course_access_removal( $remove, $subscription, $filter )
	{
		...
		if ( $filter === 'woocommerce_subscription_status_on-hold' && strpos( $backtrace, 'process_renewal' ) !== false ) {
			return false;
		}
	...
	public static function check_subscription_course_access_addition( $add, $subscription, $filter )
	{
		...
		if ( $filter === 'woocommerce_subscription_status_active' && strpos( $backtrace, 'process_renewal' ) !== false ) {
			return false;
		}
